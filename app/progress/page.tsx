  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Progress"
        subtitle="Roadmap, milestones, and phase status for Shipping Savior."
      />
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">{milestone.timeframe} &mdash; {milestone.description}</p>
              </div>
            </div>

            {/* Phase Cards */}
            <div className="space-y-2 ml-1.5 pl-4 border-l border-white/5">
              {milestone.phases.map((phase, pi) => {
                ...
                return (
                  <GlassCard key={phase.id} delay={pi * 0.03}>
                    ...
                    >
                      <div className="flex-shrink-0 w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-sm font-bold text-slate-300">
                        {phase.id}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">{phase.dateRange}</p>
                      </div>
                        ...
                        <ul className="space-y-2">
                          {phase.deliverables.map((d, j) => (
                            <li key={j} className="flex items-center gap-2 text-sm">
                              ...
                              <span className={d.done ? "text-slate-300" : "text-slate-500"}>{d.label}</span>
                            </li>
                          ))}
                        </ul>
                      </motion.div>
